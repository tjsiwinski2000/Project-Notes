...
def create(event, context):
    ...
    if res['ResponseMetadata']['HTTPStatusCode'] == 200:
        response = {
            "statusCode": 201,
            "headers": {
            "Access-Control-Allow-Origin": "https://staging.d2s3bhbyphaue.amplifyapp.com",
...
def update(event, context):
    ...
    res = dynamodb.update_item(
        ...
        UpdateExpression="set animalname=:n, image=:i, updatedAt=:u, funfacts=:f",
        ExpressionAttributeValues={
            ':n': dynamo.to_item(post['animalname']),
            ':i': dynamo.to_item(post['image']),
            ':f': dynamo.to_item(post['funfacts']),
