...
# decrypt(original_text="jgnnqb", shift_amount=2)

def encrypt_decrypt(action, orginal_text, shift_amount):
    final_text = ""
    for letter in orginal_text:
        if action == "encode":
            shifted_position = alphabet.index(letter) + shift_amount
            shifted_position %= len(alphabet)
            print(f"shift position {shifted_position}")
        if action == "decode":
            shifted_position = alphabet.index(letter) - shift_amount
    ...
    print(f"Here is the {action}d result: {final_text}")
